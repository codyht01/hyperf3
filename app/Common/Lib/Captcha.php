...
class Captcha
{
    ...
    protected string $codeSet = '2345678abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY'; // 验证码图片实例
    protected int $expire = 1800;  // 验证码字符集合
    protected bool $useZh = false;  // 验证码过期时间（s）
    ...
    protected bool $useImgBg = false;  // 中文验证码字符串
    protected int $fontSize = 25; // 使用背景图片
    protected bool $useCurve = true; // 验证码字体大小(px)
    protected bool $useNoise = true; // 是否画混淆曲线
    protected int|float $imageH = 0; // 是否添加杂点
    protected int|float $imageW = 0; // 验证码图片高度
    protected int $length = 5;  // 验证码图片宽度
    protected string $fontttf = '';  // 验证码位数
    protected array $bg = [243, 251, 254]; // 验证码字体，不设置随机获取
    protected bool $math = false; // 背景颜色
    protected string $captcha_key = "cseiwiasdj_captcha";
    /**
     * @var null
     */
    protected $response = null; //算术验证码
     ...
     * @throws \Exception
     */
    public function create(string $config = null): Psr7ResponseInterface
    {
        ...
        // 验证码使用随机字体
        $ttfPath = BASE_PATH . '/config/assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
    ...
    protected function configure(string $config = null): void
    {
        ...
        foreach ($config as $key => $val) {
            if (property_exists($this, $key)) {   //检查对象或类是否具有该属性
                $this->{$key} = $val;
            }
        }
    }
    ...
    protected function generate(): array
    {
        ...
        ApplicationContext::getContainer()->get(Redis::class)->set($this->captcha_key, $hash, $this->expire);
        return [
            'value' => $bag,
            'key' => $hash,
        ];
    ...
    protected function writeCurve(): void
    {
        ...
        $px1 = 0; // 曲线横坐标起始位置
        $px2 = mt_rand(intval($this->imageW / 2), intval($this->imageW)); // 曲线横坐标结束位置
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->im, $px + $i, intval($py + $i), $this->color); // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                $i = (int)($this->fontSize / 5);
                while ($i > 0) {
                    imagesetpixel($this->im, $px + $i, intval($py + $i), $this->color);
                    $i--;
                }
            }
        }
    ...
    {
        $client = ApplicationContext::getContainer()->get(Redis::class);

        $key = $client->get($this->captcha_key);
        if (empty($key)) {
            return false;
        }
